...
<h6>
  {t("lorem2")}<br/>{t("lorem3")} <br/>{t("lorem4")}
</h6>
<Link type='submit' to={"/login"} className='btn-style'>{t("letsGo")} </Link>
<button className='btn btn-light' style={{margin:"2px 15px 2px 15px",borderRadius:"20px"}}
 type="button" data-bs-toggle="dropdown" aria-expanded="false"
>
  <GlobeIcon className='iconGlob'  /> {currentLanguageCode =="ar" ? "اللغة" : " Language " }

</button>
<ul className="dropdown-menu" aria-labelledby="dropdownMenuButton1" style={{textAlign:"center"}}>

  {languages.map(({ code, name, country_code ,flag}) => (
    <li key={country_code}  style={{textAlign:"center"}}>
      <a
        href=""
      ...
      >
        ...
        ></span>
        <img src={flag} alt="flag" width="20px" /> {name}
      </a>
    </li>
  ))}
</ul>
    ...
    <div className="col-md-6 "style={{direction:"rtl"}}>
      <div className="loginFormSide">
        <div className='parent-svg-logo'>
        <svg id="Group_136" data-name="Group 136" xmlns="http://www.w3.org/2000/svg" width="81" height="66" viewBox="0 0 81 66">
  ...
  <path id="Path_446" data-name="Path 446" d="M81,62.825a2.884,2.884,0,0,1-.525,1.756,3.108,3.108,0,0,1-1.486,1.062,6.608,6.608,0,0,1-2.3.357,9.5,9.5,0,0,1-1.971-.179,5.7,5.7,0,0,1-1.456-.506V63.8a9.775,9.775,0,0,0,1.585.6,6.709,6.709,0,0,0,1.882.268,3.555,3.555,0,0,0,2.041-.456,1.449,1.449,0,0,0,.634-1.24,1.256,1.256,0,0,0-.258-.784,2.412,2.412,0,0,0-.842-.665,12.488,12.488,0,0,0-1.645-.714,18.976,18.976,0,0,1-1.8-.784,3.365,3.365,0,0,1-1.169-.962,2.459,2.459,0,0,1-.406-1.468,2.382,2.382,0,0,1,1.11-2.113,5.211,5.211,0,0,1,2.933-.744,8.067,8.067,0,0,1,1.833.2,8.436,8.436,0,0,1,1.6.516l-.555,1.32a10.173,10.173,0,0,0-1.437-.486,6.152,6.152,0,0,0-1.555-.2,3.371,3.371,0,0,0-1.754.377,1.162,1.162,0,0,0-.6,1.032,1.245,1.245,0,0,0,.277.833,2.723,2.723,0,0,0,.911.615q.624.288,1.645.675a13.281,13.281,0,0,1,1.763.784,3.145,3.145,0,0,1,1.139.972A2.441,2.441,0,0,1,81,62.825Z" fill="#fff"/>
      ...
      <form onSubmit={handleSignUp} className='sign-form-style'>
        <h3 className='sign-head'>{t("SignUp2")}</h3>

        
      	<input type="text" className=" input-style form-control " onChange={(e) =>setEmail(e.target.value) } placeholder={t("Email")} /><br/>
  
          <input type="password" className="input-style form-control " onChange={(e) =>setPassword(e.target.value) }  placeholder={t("Password")} /><br/>
          <input type="password" className=" input-style form-control " onChange={(e) =>setConfirmPassword(e.target.value) } placeholder={t("ConfirmPassword")}/><br/>
  
          <div style={{textAlign:"center"}}>
          <button type='submit' className='sign-sub-style'>{t("SignUp1")}</button>
          <Link type='submit' to={"/login"} className='sign-another-style'> {t("Login1")} </Link>
          </div>
          </form>
    </div>
    </div>
    ...
    <div class="custom-shape-divider-bottom-1678456400">
